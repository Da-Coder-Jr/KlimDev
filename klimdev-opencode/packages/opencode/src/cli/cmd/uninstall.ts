...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall klimdev and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall KlimDev")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      npm: ["npm", "uninstall", "-g", "klimdev"],
      pnpm: ["pnpm", "uninstall", "-g", "klimdev"],
      bun: ["bun", "remove", "-g", "klimdev"],
      yarn: ["yarn", "global", "remove", "klimdev"],
      brew: ["brew", "uninstall", "opencode"],
      choco: ["choco", "uninstall", "opencode"],
      scoop: ["scoop", "uninstall", "opencode"],
    }
  ...
  UI.empty()
  prompts.log.success("Thank you for using KlimDev!")
}
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".klimdev"))
    ) {
      continue
    }
