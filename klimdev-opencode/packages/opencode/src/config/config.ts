...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      ...
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "klimdev")
      default:
        return "/etc/klimdev"
    }
  }
  ...
  const MANAGED_PLIST_DOMAIN = "ai.klimdev.managed"
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.klimdev/command/", "/.klimdev/commands/", "/.opencode/command/", "/.opencode/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.klimdev/agent/", "/.klimdev/agents/", "/.opencode/agent/", "/.opencode/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://klimdev.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://klimdev.ai/docs/agents"),
      provider: z
  ...
  function globalConfigFile() {
    const candidates = ["klimdev.jsonc", "klimdev.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in klimdev config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://klimdev.ai/config.json"
              const updated = original.replace(/^\s*\{/, '{\n  "$schema": "https://klimdev.ai/config.json",')
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://klimdev.ai/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://klimdev.ai/config.json"
