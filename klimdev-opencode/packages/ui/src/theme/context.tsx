...
import klimdevThemeJson from "./themes/klimdev.json"
const klimdevTheme = klimdevThemeJson as import("./types").DesktopTheme
...
const STORAGE_KEYS = {
  THEME_ID: "klimdev-theme-id",
  COLOR_SCHEME: "opencode-color-scheme",
  THEME_CSS_LIGHT: "klimdev-theme-css-light",
  THEME_CSS_DARK: "klimdev-theme-css-dark",
} as const
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { defaultTheme?: string; onThemeApplied?: (theme: DesktopTheme, mode: "light" | "dark") => void }) => {
    const themeId = normalize(read(STORAGE_KEYS.THEME_ID) ?? props.defaultTheme) ?? "klimdev"
    const colorScheme = (read(STORAGE_KEYS.COLOR_SCHEME) as ColorScheme | null) ?? "system"
    const mode = colorScheme === "system" ? getSystemMode() : colorScheme
    const [store, setStore] = createStore({
      themes: {
        "oc-2": oc2Theme,
        "klimdev": klimdevTheme,
    ...
    onMount(() => {
      ...
      const savedTheme = normalize(rawTheme ?? props.defaultTheme) ?? "klimdev"
